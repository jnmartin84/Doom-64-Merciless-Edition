...
unsigned char  __attribute__((aligned(16))) rndtable[256] = {
...
// dynamic resolution switching, this needs some refinement
#if 0        
        if(((OS_CYCLES_TO_NSEC(last_iter_count)/1000) > 50000) && (SCREEN_WD == 640))
        {
            SCREEN_WD = 320;
            P_RefreshVideo();
        }
        else if(((OS_CYCLES_TO_NSEC(last_iter_count)/1000) < 50000) && (SCREEN_WD == 320))
        {
            SCREEN_WD = 640;
            P_RefreshVideo();
        }
