...
static void (*FuncQueueArr[15])(void) =
{
...
static int func_num_arr = 0;						
static QUEUE_DATA_MEMORY queue_data_mem;			
static char *queue_write_ptr = queue_data_mem.data; 
static char *queue_read_ptr;						

void queue_memcpy(void *dest, void *src, int size)
{
	char *d, *s;
...
void queue_the_data(void *src, int size)
{
	int memsize;
...
void unqueue_the_data(void *dest, int size)
{
	queue_memcpy(dest, queue_read_ptr, size);
	queue_read_ptr += size;
}

void process_function_queue(void)
{
	char *func;
	int i;
